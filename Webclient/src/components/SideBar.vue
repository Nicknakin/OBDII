            ...
            <div class="mt-5 flex flex-1 flex-col">
                <nav class="flex-1 space-y-1 px-2 pb-4">
                    <router-link v-for="item in navigation" :key="item.name" :to="item.href"
                        ...
                        <component :is="item.icon" class="mr-3 h-6 w-6 flex-shrink-0 text-gray-300"
                            aria-hidden="true" />
                        {{ item.name }}
                    </router-link>
                </nav>
...
const navigation = [
    ...
    { name: "View Database Query History", href: "/history?count=10&start=1", icon: DatabaseIcon, current: false },
]
</script>
